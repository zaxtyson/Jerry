...
void testRawTimeQueue() {
    ...
    Date now = Date::now();
    timerQueue.addTimer(now.addSeconds(1), show("1s"));
...
void testRunAt() {
    ...
    loop.runAt(Date::now().addSeconds(3), show("3s"));
    loop.runAt(Date::now().addSeconds(0.5), show("0.5s"));
    loop.runAt(Date::now().addSeconds(5), show("5s"));
    loop.runAt(Date::now().addSeconds(1), show("1s"));

    std::thread t1([&loop]() {
        std::this_thread::sleep_for(1s);
        Date d = Date::now();
        loop.runAt(d.addSeconds(1), show("other thread 1"));
        loop.runAt(d.addSeconds(3), show("other thread 2"));
    });
    t1.detach();
    loop.loop();
}
